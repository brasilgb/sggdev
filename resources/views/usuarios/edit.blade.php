...
@section('content')
    ...
    <div class="card shadow-sm">
        ...
        <form id="formusuario" action="{{ route('usuarios.update', ['usuario' => $usuario->id]) }}" method="post" autocomplete="off">
...
{{--
                ...
                <div class="form-group row">
                    <label for="funcao" class="col-sm-3 col-form-label text-left">Função <span
                            class="text-danger">*</span></label>
                    <div class="col-sm-7">
                        <select @if($usuario->id == 1) disabled @endif id="funcao" type="funcao" class="custom-select" name="funcao">
                            @foreach ($funcao as $key => $value)
                                <option value="{{ $usuario->funcao }}" @if($usuario->funcao == $key) selected @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                        @error('funcao')
                            <div class="alert alert-danger">{{ $message }}</div>
