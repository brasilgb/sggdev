...
<div class="card shadow-sm">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb pt-1 pb-1 float-right bg-transparent">
                        ...
                        <li class="breadcrumb-item active">
                            @if (isset($busca)) <a
                                href="{{ route('usuarios.index') }}">Usuários</a> / Busca @else Usuários
                            @endif
                        </li>
                    </ol>
                </nav>
    ...
    <div class="card-body p-2">
        <div class="table-responsive">
            ...
            <table id="tb-usuarios" class="table table-condensed table-striped mb-0">
                <thead>
                    <tr class="text-left">
                        <th>Nome</th>
                        <th>Usuário</th>
                ...
                <tbody>
                    @forelse ($usuarios as $usuario)
                        <tr class="text-left">
                            ...
                            <td class="align-middle">{{ $usuario->funcao == 0 ? 'Administrador' : 'Operador' }}</td>
                            <td class="align-middle">

                                <button
                                    onclick="window.location='{{ route('usuarios.edit', ['usuario' => $usuario->id]) }}'"
                                    id="situacao" class="btn btn-primary border border-white shadow"><i
                                        class="fa fa-edit"></i> Editar</button>

                                <button @if($usuario->id == 1) disabled @endif class="btn btn-danger border border-white shadow" data-toggle="modal"
                                    onclick="deleteData({{ $usuario->id }})" data-target="#DeleteModal"><i
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="alert alert-danger text-left"><i class="fa fa-exclamation-triangle"></i>
                                ...
                                @else
                                    Não há usuários cadastrados, adicione um usuário para continuar!
                                @endif
                            </td>
                        </tr>
                    @endforelse
