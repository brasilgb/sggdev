...
@section('content')
    ...
    <div class="card shadow-sm">
        ...
        <form id="formdespesa" action="{{ route('despesas.update', ['despesa' => $despesa->id_despesa]) }}" method="post"
            autocomplete="off">
            <div class="card-body px-4">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-7">
                        <input id="data1" type="text" class="form-control" name="vencimento"
                            value="{{ old('vencimento', date('d/m/Y', strtotime($despesa->vencimento))) }}">
                        @error('vencimento')
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-7">
                        <input id="descritivo" type="text" class="form-control" name="descritivo"
                            value="{{ old('descritivo', $despesa->descritivo) }}">
                        @error('descritivo')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                @endphp
                <div class="form-group row">
                    <label for="situacao" class="col-sm-3 col-form-label text-left">
                        Situação <span class="text-danger">*</span>
                    </label>
                    <div class="col-sm-7">
                        <select name="situacao" id="situacao" class="custom-select">
                            @foreach ($situacao as $key => $value)
                                <option value="{{ $key }}" @if ($key == $despesa->situacao) selected @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-7">
                        <textarea rows="3" id="observacao" type="text" class="form-control"
                            name="observacao">{{ old('observacao', $despesa->observacao) }}</textarea>
                        @error('observacao')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
