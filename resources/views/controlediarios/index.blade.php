                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb pt-1 pb-1 float-right bg-transparent">
                            ...
                            <li class="breadcrumb-item active">
                                @if (isset($busca)) <a
                                    href="{{ route('controlediarios.index') }}">Controle diário</a> / Busca @else
                                    Controle diário
                                @endif
                            </li>
                        </ol>
        ...
        <div class="card-header">
            <div class="row">
                <div class="col text-left">
                    <button onclick="window.location='{{ route('controlediarios.create') }}'"
                    ...
                    <tbody>
                        @forelse ($controlediarios as $controlediario)
                            <tr class="text-left">
                                ...
                                <td class="align-middle">{{ $controlediario->temperatura_max }}</td>
                                <td class="align-middle">{{ $controlediario->temperatura_min }}</td>
                                <td class="align-middle">{{ $controlediario->umidade }}</td>
                                <td class="align-middle">{{ $causas($controlediario->leitura_agua) }}</td>
                                <td class="align-middle">{{ $controlediario->consumo_total }}</td>
                                <td class="align-middle">{{ $causas($controlediario->consumo_ave) }}</td>
                                ...
                                <td class="align-middle">
                                    ...
                                    <button class="btn btn-danger border border-white shadow" data-toggle="modal"
                                        onclick="deleteData({{ $controlediario->id_controle }})"
